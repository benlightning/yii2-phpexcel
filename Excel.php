 ...
 * ~~~
 * 
 * $data = \app\libs\excel\Excel::import($fileName, $config); // $config is an optional
 * 
 * $data = \app\libs\excel\Excel::widget([
 * 		'mode' => 'import',
 * 		'fileName' => $fileName,
 ...
 * ]);
 * 
 * $data = \app\libs\excel\Excel::import($fileName, [
 * 		'setFirstRecordAsKeys' => true, // if you want to set the keys of record column with first record, if it not set, the header with use the alphabet column on excel.
 * 		'setIndexSheetByName' => true, // set this if your excel data with multiple worksheet, the index of array will be set with the sheet name. If this not set, the index will use numeric.
 * 		'getOnlySheet' => 'sheet1', // you can set this property if you want to get the specified sheet from the excel data with multiple worksheet.
 * 	]);
 ...
 * ]);
 * 
 * $data = \app\libs\excel\Excel::import([
 * 			'file1' => $fileName1,
 * 			'file2' => $fileName2,
 * 			'file3' => $fileName3,
 * 		], [
...
class ExcelWidget extends \yii\base\Widget {
    ...
    /**
     * @var string name for file extension to export or save.
     */
    public $extensionName = '.xls';
    ...
    public function getFileName() {
        $fileName = date('YmdHis').$this->extensionName;//'exports.xls';
        if (isset($this->fileName)) {
            $fileName = $this->fileName;
            if (strpos($fileName, $this->extensionName) === false){
     ...
     * ~~~
     * 
     * $data = \app\libs\excel\Excel::import($fileName, ['setFirstRecordAsKeys' => true]);
     * 
     * ~~~
     * 
